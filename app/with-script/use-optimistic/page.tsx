...
// Use in transition for async updates
startTransition(async () => {
  ...
  setData(prev => [...prev, newItem]);
      ...
      <div className="card">
        <h3 className="card-title">
          <span>🧪</span>
          Live Demo - Optimistic Todo List
        </h3>
        <p className="card-description">
          Toggle todos or add new ones. Notice the instant feedback - items update 
          immediately while the server request happens in the background.
        </p>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
          ...
          />
          <button className="btn-primary" onClick={handleAddTodo} disabled={!newTodo.trim()}>
            Add Todo
          </button>
        </div>
            ...
            >
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  border: "2px solid",
                  borderColor: todo.completed ? "#00ff7f" : "rgba(255,255,255,0.3)",
                  background: todo.completed ? "#00ff7f" : "transparent",
                  display: "flex",
                  alignItems: "center",
              ...
              {todo.pending && (
                <span className="status-badge status-pending">Syncing...</span>
              )}
            </li>
          ))}
        </ul>
