...
async function getSlowData(delay: number, label: string) {
  ...
  return { label, data: `Data loaded after ${delay}ms`, timestamp: new Date().toISOString() };
}

async function SlowComponent({ delay, label }: { delay: number; label: string }) {
  const result = await getSlowData(delay, label);
  return (
    <div style={{ padding: "1rem", background: "rgba(0, 255, 127, 0.1)", border: "1px solid rgba(0, 255, 127, 0.2)", borderRadius: 8 }}>
      <h4 style={{ margin: "0 0 0.5rem", color: "#00ff7f" }}>{result.label}</h4>
      <p style={{ margin: 0, color: "rgba(255,255,255,0.7)", fontSize: "0.9rem" }}>{result.data}</p>
      <p style={{ margin: "0.5rem 0 0", fontSize: "0.75rem", color: "rgba(255,255,255,0.4)" }}>Loaded: {result.timestamp}</p>
    </div>
  );
}
...
function LoadingFallback({ label }: { label: string }) {
  return (
    <div className="loading" style={{ padding: "1rem", background: "rgba(255, 193, 7, 0.1)", border: "1px dashed rgba(255, 193, 7, 0.3)", borderRadius: 8 }}>
      <h4 style={{ margin: "0 0 0.5rem", color: "#ffc107" }}>⏳ Loading {label}...</h4>
      <div style={{ height: 16, background: "rgba(255,255,255,0.1)", borderRadius: 4, width: "60%" }} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <p style={{ color: "rgba(255, 255, 255, 0.7)", marginBottom: "2rem" }}>
        Suspense boundaries allow components to "wait" for async data while showing fallback UI.
      </p>
...
export default function Page() {
  return (
    <Suspense fallback={<Loading />}>
      ...
      <div className="card">
        <h3 className="card-title"><span>🧪</span> Live Demo - Parallel Loading</h3>
        <p className="card-description">Each component fetches data independently. Watch them stream in!</p>
      ...
      <div className="card">
        <h3 className="card-title"><span>🔗</span> Nested Suspense</h3>
        <p className="card-description">Inner boundaries resolve independently of outer ones.</p>
        <Suspense fallback={<LoadingFallback label="Outer Container" />}>
          <div style={{ padding: "1rem", background: "rgba(0, 217, 255, 0.05)", border: "1px solid rgba(0, 217, 255, 0.2)", borderRadius: 8 }}>
      ...
      <div className="card">
        ...
        <ul style={{ color: "rgba(255, 255, 255, 0.7)", lineHeight: 1.8 }}>
          ...
          <li>Better UX - meaningful loading states per section</li>
