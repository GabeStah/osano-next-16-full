...
async function submitForm(formData: FormData): Promise<{ success: boolean; message: string }> {
  ...
  return { success: true, message: `Successfully submitted for ${email}${subscribe ? " with newsletter subscription" : ""}!` };
}
  ...
  return (
    <div>
      <button type="submit" className="btn-primary" disabled={pending}>
        {pending ? <><span className="loading" style={{ marginRight: "0.5rem" }}>⏳</span>Submitting...</> : "Submit Form"}
      </button>
      {pending && (
        <div style={{ marginTop: "1rem" }}>
          <span className="status-badge status-pending">Form is pending...</span>
          <p style={{ margin: "0.5rem 0 0", fontSize: "0.8rem", color: "rgba(255,255,255,0.5)" }}>Method: <code>{method}</code></p>
        </div>
      )}
    </div>
  );
...
function FormInput({ name, type = "text", placeholder, required = false }: { name: string; type?: string; placeholder: string; required?: boolean }) {
  const { pending } = useFormStatus();
  return <input type={type} name={name} placeholder={placeholder} required={required} disabled={pending} style={{ width: "100%", maxWidth: 400, opacity: pending ? 0.6 : 1 }} />;
}

function FormCheckbox({ name, label }: { name: string; label: string }) {
  const { pending } = useFormStatus();
  return (
    <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: pending ? "not-allowed" : "pointer", opacity: pending ? 0.6 : 1 }}>
...
export default function FormStatusContent() {
  ...
  return (
    <div>
      ...
      <p style={{ color: "rgba(255, 255, 255, 0.7)", marginBottom: "2rem" }}>
        The <code>useFormStatus</code> hook lets child components access the pending state of their parent form.
      </p>
...
<form action={serverAction}>
  ...
  <SubmitButton />
      ...
      <div className="card">
        ...
        <p className="card-description">Submit the form to see child components react to the form's pending state.</p>
        <form action={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {result && (
            <div className="demo-box" style={{ borderColor: "rgba(0, 255, 127, 0.3)" }}>
              ...
              <p style={{ margin: "0.75rem 0 0", color: "rgba(255,255,255,0.8)" }}>{result.message}</p>
            </div>
          )}
        </form>
      </div>
      ...
      <div className="card">
        <h3 className="card-title"><span>📋</span> useFormStatus Returns</h3>
