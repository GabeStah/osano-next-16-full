...
import { submitFormAction, type FormState } from "../_actions/formActions";

const initialState: FormState = { message: "", status: "idle" };

export default function UseActionStateContent() {
    ...
    <form action={formAction}>
      ...
      <div className="card">
        ...
        <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div>
            <label htmlFor="name" style={{ display: "block", marginBottom: "0.5rem", fontSize: "0.9rem" }}>Name</label>
            <input type="text" id="name" name="name" placeholder="Enter your name" style={{ width: "100%", maxWidth: 400 }} disabled={isPending} />
          </div>
          <div>
            <label htmlFor="email" style={{ display: "block", marginBottom: "0.5rem", fontSize: "0.9rem" }}>Email</label>
            <input type="email" id="email" name="email" placeholder="you@example.com" style={{ width: "100%", maxWidth: 400 }} disabled={isPending} />
          </div>
          <div>
          ...
          {state.status !== "idle" && (
            <div className="demo-box" style={{ borderColor: state.status === "success" ? "rgba(0, 255, 127, 0.3)" : "rgba(255, 82, 82, 0.3)" }}>
              <span className={`status-badge ${state.status === "success" ? "status-success" : "status-error"}`}>
                {state.status === "success" ? "✓ Success" : "✗ Error"}
              </span>
              <p style={{ margin: "0.75rem 0 0", color: "rgba(255,255,255,0.8)" }}>{state.message}</p>
            </div>
          )}
        </form>
      </div>
